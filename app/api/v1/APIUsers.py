...
from app.Servises.users_servises import create, authorization_user, get_id, get_all_users, delete, update
...
async def registration(user: UsersRegistration, session: SessionDepend):
    new_user = await create(session=session, user=user)
...
async def get_user_by_id(user_id: int, session: SessionDepend):
    user = await get_id(session=session, user_id=user_id)
    return {"detail": user}
...
async def update_user(session: SessionDepend, user_id: int, user_update: UsersUpdata):
    user = await update(session=session, user_id=user_id, user_update=user_update)
    return {"detail": user}


@router.delete("/delete/{user_id}/")
async def delete_user(user_id: int, session: SessionDepend):
    await delete(session=session, user_id=user_id)
